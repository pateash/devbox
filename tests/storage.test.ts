...
describe('repository-scoped work', () => {
  it('allows reusable assignments and removes only stale workspace rows', () => {
    ...
    db.setAssignedRepositories(first.id, ['octocat/api', 'octocat/web']); db.setAssignedRepositories(second.id, ['octocat/api'])
    ...
    db.setAssignedRepositories(first.id, ['octocat/api'])
    expect(db.assignedRepositories(second.id)).toEqual(['octocat/api'])
    expect(db.workItems(first.id).map((item) => item.title)).toEqual(['API PR'])
