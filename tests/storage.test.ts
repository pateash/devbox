...
describe('repository-scoped work', () => {
  it('keeps a workspace repository attached when another GitHub integration is added', () => {
    ...
    const secondIntegrationId = db.createGlobalGitHub('Work GitHub', Buffer.from('new-token'), ['pateash/devbox'])
    expect(secondIntegrationId).not.toBe(integrationId)
    expect(db.globalGitHubs()).toHaveLength(2)
