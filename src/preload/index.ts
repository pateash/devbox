...
const api: DevBoxApi = {
  ...
  backups: { status: () => ipcRenderer.invoke('backups:status'), repositories: () => ipcRenderer.invoke('backups:repositories'), configure: (repositoryId, acknowledgement) => ipcRenderer.invoke('backups:configure', repositoryId, acknowledgement), run: () => ipcRenderer.invoke('backups:run'), restoreLatest: (confirmation) => ipcRenderer.invoke('backups:restoreLatest', confirmation) },
