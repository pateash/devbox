...
export function registerIpc(store: DevBoxStore, fixtureMode: boolean, github: GitHubService): void {
  ...
  ipcMain.handle('github:setAssignedRepositories', (_e, workspaceId, repositories) => { store.setAssignedRepositories(id.parse(workspaceId), z.array(z.string().regex(/^[^/]+\/[^/]+$/)).parse(repositories)); changed() })
