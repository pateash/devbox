...
export class JiraService {
  ...
  async connect(input:{deployment:'cloud'|'data-center';baseUrl:string;email?:string;token:string;name?:string}):Promise<{integration:NonNullable<ReturnType<DevBoxStore['globalJira']>>;projects:JiraProject[]}> {
    ...
    const displayName=input.name?.trim() || (user.displayName ?? user.emailAddress ?? user.name ?? 'Jira account')
    this.store.createGlobalJira(displayName,this.vault.encrypt(credential),{deployment:input.deployment,baseUrl,email:input.deployment==='cloud'?input.email!.trim():undefined})
    return {integration:this.store.globalJira()!,projects}
  }
  async availableProjects():Promise<JiraProject[]> { const global=this.store.globalJira(); if(!global) throw new Error('Jira is not connected.'); const credential=this.vault.decrypt(this.store.globalJiraToken(global.id)); const auth=global.deployment==='cloud'?`Basic ${Buffer.from(credential).toString('base64')}`:`Bearer ${credential}`; return this.projects(global.baseUrl,auth) }
  async refresh(workspaceId:string):Promise<void> { const integration=this.store.jiraIntegration(workspaceId); const global=this.store.globalJira(); if(!global) throw new Error('Jira is not connected.'); if(!integration?.projects.length) return; try { const credential=this.vault.decrypt(this.store.globalJiraToken(global.id)); const auth=global.deployment==='cloud' ? `Basic ${Buffer.from(credential).toString('base64')}` : `Bearer ${credential}`; const issues=await this.issues(global.baseUrl,auth,integration.projects.map((project)=>project.key)); const items=issues.filter((issue)=>issue.fields.status?.statusCategory?.key!=='done').map((issue):Omit<WorkItem,'id'|'provider'>=>({repository:issue.key.split('-')[0]??'Jira',title:`${issue.key} · ${issue.fields.summary??'Untitled issue'}`,reason:'Assigned to you',priority:priority(issue.fields.priority?.name),url:`${global.baseUrl}/browse/${encodeURIComponent(issue.key)}`,updatedAt:issue.fields.updated??new Date().toISOString(),kind:'jira-issue',status:issue.fields.status?.name})); this.store.replaceJiraWork(integration.id,items) } catch(error) { const message=error instanceof Error?error.message:'Jira sync failed.'; const state=error instanceof JiraApiError&&(error.status===401||error.status===403)?'needs_reauth':'error'; this.store.globalIntegrationError(global.id,state,message); if(!integration.id.includes(':')) this.store.markJiraError(integration.id,state,message); throw error } }
