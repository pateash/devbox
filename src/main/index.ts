...
app.whenReady().then(() => { const store = new DevBoxStore(join(app.getPath('userData'), 'devbox.sqlite')); const bridge=startDevBrowserBridge(store); const github=new GitHubService(store); registerIpc(store, fixtureMode, github); Menu.setApplicationMenu(Menu.buildFromTemplate([{ label: 'DevBox', submenu: [{ role: 'quit' }] }, { role: 'editMenu' }])); tray = new Tray(nativeImage.createEmpty()); tray.setToolTip('DevBox'); tray.on('click', () => { if (mainWindow) { mainWindow.show(); mainWindow.focus() } }); globalShortcut.register('Alt+Space', () => { mainWindow?.show(); mainWindow?.focus() }); createWindow(); app.on('activate', () => { if (!mainWindow) createWindow() }); app.on('before-quit', (event) => { const backup=store.backupStatus(); if(!completingQuitBackup && backup.repository && backup.dirty) { event.preventDefault(); completingQuitBackup=true; void Promise.race([github.backupNow().catch(()=>undefined),new Promise((resolve)=>setTimeout(resolve,3000))]).finally(()=>app.quit()); return } bridge?.close(); store.close(); globalShortcut.unregisterAll() }) })
